...
def load_map(self):
    def load_info(map, author, self):
        map_name = map.get()
        author = author.get()
        load(self.buttons, str(map_name) + " by " + str(author), self)
        screen.destroy()
    ...
    save_button = Button(screen, text="Load", width="30", height="2", command=lambda: load_info(map_entry, author_entry, self), bg="grey")
    save_button.place(x=15, y=290)
