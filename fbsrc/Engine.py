...
data = {}
with open("data.MBD", "r") as f:
    tmp_data = f.read()
    tmp_data = tmp_data.split("\n")
    for config in tmp_data:
        if config.startswith("#") or config.startswith("[") or config == "":  # # and [ are description starters
            continue
        data[config.split("=")[0]] = config.split("=")[1]
...
class Window:
    def __init__(self, *, load_map=False, map_name="", play=False):
        ...
        # Title screen
        self.text_label = tk.Label(self.frame3, text=data["title"])
...
if __name__ == "__main__":
    window = Window(load_map=True, map_name=data["map_name"], play=True)
    window.mainloop()
